...
def getCryptoProve(ccurrencylist):
    try:
        ...
        cryptocurrencylist = json.loads(apidata.text)["data"]
    ...
    try:
        ...
        for fcurrency in fcurrencylist:
            ...
            if(ret_fcurrencylist[len(ret_fcurrencylist) - 1] != fcurrency.upper()):
                print("\n!!! Couldn't find \"" + fcurrency +"\" as a valid currency. Please recheck. !!!\n")
    ...
    try:
        ...
        for id in ccidlist:
            ...
            for fc in fclist:
                ...
                price = round(float(json.loads(apidata.text)["data"]["quotes"][fc]["price"]), 2)
...
def get24hChange(ccidlist):
    try:
        ...
        for id in ccidlist:
            ...
            change = json.loads(apidata.text)["data"]["quotes"]["USD"]["percent_change_24h"]
            ret_data[id] = change
        return ret_data
    except Exception as e:
        print(str(e))
...
def get7dChange(ccidlist):
    try:
        ...
        for id in ccidlist:
            apidata = requests.get(ccapi_url + "/" + str(id) + "/")
            change = json.loads(apidata.text)["data"]["quotes"]["USD"]["percent_change_7d"]
            ret_data[id] = change
        return ret_data
    except Exception as e:
        print(str(e))
...
def get1hChange(ccidlist):
    try:
        ...
        for id in ccidlist:
            apidata = requests.get(ccapi_url + "/" + str(id) + "/")
            change = json.loads(apidata.text)["data"]["quotes"]["USD"]["percent_change_1h"]
            ret_data[id] = change
        return ret_data
    except Exception as e:
        print(str(e))
